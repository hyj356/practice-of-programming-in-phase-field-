program main
  ! gfortran mod_case_5_V1.f90 main.f90 -O2; ./a.exe
  use case_5_V2, only: write_vtk_file, free_energy_v2, &
  ...
  ! 决定初始细胞数量和大小的参数
  real(WP), parameter :: R = 25.0_WP  !< ncell为2时, R为25, ncell为80的时候, R为12
  ! 参与仿真计算的不变量
  ...
  integer :: ncell = 2  !< 模型中细胞的数量, 这个值在当前的模拟中只能为80或者2
