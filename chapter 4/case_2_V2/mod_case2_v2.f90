module case_2_v2
  ...
  public :: WP, stdout, write_vtk_file, init_grain_micro, &
            free_energ_fd_ca_v1, write_eta, eyes, toepliz, kron
...
contains
  ...
  pure function free_energ_fd_ca_v2(ngrain, etas, Nx, Ny, igrain) result(dfdeta)
    integer, intent(in) :: ngrain
    integer, intent(in) :: igrain
    ...
    deallocate(sum_)
  end function free_energ_fd_ca_v2

  pure subroutine laplacian(Nx, Ny, dx, dy, grad)
    integer, intent(in) :: Nx, Ny
    real(WP), intent(in) :: dx, dy
    real(WP), allocatable,intent(inout) :: grad(:, :)
    real(WP), allocatable :: r(:)
    real(WP), allocatable :: T(:, :)
    real(WP), allocatable :: E(:, :)
    integer :: ii, jj, kk, i, NxNy

    if (.not. allocated(grad)) allocate(grad(Nx*Ny, Nx*Ny))
    allocate(r(Nx), source=0.0_WP)
    r(1) = 2.0_WP; r(2) = -1.0_WP
    T = toepliz(r)
    E = eyes(Nx)
    NxNy = Nx*Ny
    ...
    do i = 1, Nx
      ii = (i-1)*Nx + 1
      jj = ii + Nx - 1
      grad(ii, jj) = 1.0_WP;  grad(jj, ii) = 1.0_WP;
      kk = NxNy - Nx + i
      grad(i, kk) = 1.0; grad(kk, i) = 1.0
    end do
    grad = grad / (dx*dy)
  end subroutine laplacian

  pure function kron(mat1, mat2) result(ret)
    real(WP), intent(in) :: mat1(:, :)
    real(WP), intent(in) :: mat2(:, :)
    real(WP), allocatable :: ret(:, :)
    integer :: m, n, p, q
    integer :: i, j, k, l

    m = size(mat1, dim=1); n = size(mat1, dim=2) 
    p = size(mat2, dim=1); q = size(mat2, dim=2) 

    allocate(ret(m*p, n*q))

    do i = 1, m
      do j = 1, n
        do k = 1, p
          do l = 1, q
            ret((i-1)*p + k, (j-1)*q + l) = mat1(i, j) * mat2(k, l)
          end do
        end do
      end do
    end do 
  ...
  pure function toepliz(r) result(epliz)
    real(WP), intent(in) :: r(:)
    real(WP), allocatable :: epliz(:, :)
    integer :: N, i

    N = size(r)
    allocate(epliz(N, N), source=0.0_WP)
    do i = 1, N
      epliz(i, i:N) = r(1:N-i+1)
      epliz(i, 1:i) = r(i:1:-1)
    end do

  end function toepliz

  pure function eyes(N) result(ret)
    integer, intent(in) :: N
    real(WP), allocatable :: ret(:, :)
    integer :: i

    allocate(ret(N, N), source = 0.0_WP)
    do i = 1, N
      ret(i, i) = 1.0_WP
    end do
  end function
end module case_2_v2