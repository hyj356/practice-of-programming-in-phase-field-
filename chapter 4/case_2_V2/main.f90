program main
  ! gfortran .\mod_case2_v2.f90 .\main.f90 -O2; ./a.exe
  use case_2_v2, only: WP, stdout, init_grain_micro, write_vtk_file,&
  ...
  do k = 1, nstep
    ...
    do igrain = 1, ngrain
      ...
      if(glist(igrain) == 1) then
        ...
        ! 修正数值误差, 利用Fortran的掩码操作可以轻松完成
        where(eta >= 0.9999_WP) eta = 0.9999_WP
        where(eta <= 0.0001_WP) eta = 0.0001_WP
        ! 计算第igrain个晶粒的体积分数
