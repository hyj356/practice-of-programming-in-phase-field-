module case_10_V1
    ...
    public :: write_vtk_file, prepare_fft, fft_2d, ifft_2d, &
                ...
                micro_ch_pre, FeCr_chem_potent_v1, FeCr_chem_potent_v2, &
                dislo_strain, free_energ_ch
...
contains
    pure subroutine dislo_strain(ed11, ed22, ed12, Nx, Ny, idislo)
        real(wp), intent(inout) :: ed11(:, :), ed12(:, :), ed22(:, :)
        integer, intent(in) :: Nx, Ny
        integer, intent(in) :: idislo
        ! 函数的局部变量
        integer :: Ny2
        integer :: ndipoles     !< 其值等于34
        integer :: ndipolee     !< 其值等于94
        integer :: i, j, jj
        integer :: ndis, idis
        ...
        if(idislo == 1) then
            do i = 1, Nx
                if (i >= 34 .and. i <= 94) then
                    ed12(i,Ny2) = 5.0e-1_wp
                end if
            end do
        end if
        ...
        if(idislo == 2) then
            ...
            do idis = 1, ndis
                ...
                do j = 1, Ny
                    do i = 1, Nx
                        if (i >= ndipoles .and. i <= ndipolee) then
                            if (j == jj) ed12(i,j) = 5.0e-1_wp
                        end if
                    end do
                end do
            end do
    ...
    pure subroutine FeCr_chem_potent_v2(cr, dfdcr, Nx, Ny, tempr)
        !! 计算化学能函数f(c)的导数
        real(WP), intent(in) :: cr(:, :) !< 浓度数组, 形状为con(Nx, Ny)
        real(wp), intent(in) :: tempr    !< 退火温度
        integer, intent(in) :: Nx, Ny       !< 指明计算几行几列的节点
        real(wp), allocatable, intent(inout) :: dfdcr(:, :)
        real(WP) :: RT

        if (.not. allocated(dfdcr)) allocate(dfdcr(Nx, Ny))
        RT = 8.314462_wp * tempr
        dfdcr(:, :) = (-cr(:, :)*(20500.0_wp-9.68_wp*tempr)+(1.0_wp-cr(:, :)) &
                *(20500.0_wp-9.68_wp*tempr)+(log(cr(:, :))-log(1.0_wp-cr(:, :)))*RT)/RT

    end subroutine FeCr_chem_potent_v2

    pure function FeCr_chem_potent_v1(cr, i, j, tempr) result(dfdcr)
        !! 计算化学能函数f(c)的导数
        real(WP), intent(in) :: cr(:, :) !< 浓度数组, 形状为con(Nx, Ny)
        real(wp), intent(in) :: tempr    !< 退火温度
        ...
        RT = 8.314462_wp * tempr
        dfdcr = (-cr(i,j)*(20500.0_wp-9.68_wp*tempr)+(1.0_wp-cr(i,j)) &
                *(20500.0_wp-9.68_wp*tempr)+(log(cr(i,j))-log(1.0_wp-cr(i,j)))*RT)/RT

    end function FeCr_chem_potent_v1
