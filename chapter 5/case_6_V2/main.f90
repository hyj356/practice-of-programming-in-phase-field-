program hello
    use case_6_V1, only: wp, cwp, stdout, fft_2d, ifft_2d, micro_ch_pre,&
                         prepare_fft, write_vtk_file, free_energ_ch_v2, coefA
    implicit none
    ...
    complex(cwp), allocatable :: numer(:, :)
    real(wp), allocatable :: denom(:, :)
    integer :: istep, i, j

    ! 分配内存
    allocate(conk(Nx, Ny), dfdcon(Nx, Ny), dfdconk(Nx, Ny))
    allocate(numer(Nx, Ny), denom(Nx, Ny))
    ! 初始化浓度数组
    ...
    do istep = 1, nstep
        ! 计算自由能导数
        dfdcon(:, :) = free_energ_ch_v2(con)
        ! 进行傅里叶变换
        call fft_2d(input=dfdcon, output=dfdconk, Nx=Nx, Ny=Ny)
        ! 计算常数
        numer = dt*mobility*k2(:, :)*dfdconk(:, :)
        denom = 1.0_wp + dt*coefA*mobility*grad_coef*k4(:, :)
        ! 进行时间积分
